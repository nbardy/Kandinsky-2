...
def colored_noise(
    ...
    f_h = math.pi * torch.arange(shape[-2],
                                 device=device, dtype=dtype) / shape[-2]
    f_w = math.pi * torch.arange(shape[-1],
                                 device=device, dtype=dtype) / shape[-1]
...
def gamma_offset_noise(latents, discount=1.0):
    ...
    g2 = torch.clamp(
        random_gamma(
            (latents.shape[0], latents.shape[1], 1, 1), alpha=0.5, beta=10),
        0,
        1,
    ).to(device)
...
def random_high_freq(latents):
    ...
    for i in range(B):
        for j in range(C):
            ...
            rotated_checkerboard = rotate_3d_and_project(
                checkerboard, random_angle)
    ...
    for prompt in prompts_with_size:
        for (schedule_name, scheduler, steps, gscale) in schedulers:
            ...
            with torch.autocast(device_type="cuda", dtype=vae.dtype):
                ...
                label_negative = (
                    "None" if negative_prompt is None else str(
                        negative_prompt[0:20])
                )
                ...
                wandb.log(
                    {label: [wandb.Image(image, caption=label)
                             for image in images]},
                    step=step,
                )
    ...
    if args.lr_scheduler == "polynomial":
        ...
        class LogarithmicLR(torch.optim.lr_scheduler._LRScheduler):
            def __init__(self, optimizer, base_lr, max_lr, num_iterations, last_epoch=-1, verbose=False):
                ...
                super(LogarithmicLR, self).__init__(
            ...
            def get_lr(self):
                ...
                new_lr = self.base_lr + \
                    (lr_diff * np.log10(1 + 9 * step_ratio))
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.pink_noise:
                    ...
                    noise = gamma_offset_noise(
                        latents, discount=args.gamma_offset_noise)
            ...
            if accelerator.is_main_process:
                if (
                    ...
                    save_model(accelerator, unet,
                               text_encoder, args, global_step)
